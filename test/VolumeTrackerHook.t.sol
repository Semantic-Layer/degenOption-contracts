...
contract TestVolumeTrackerHook is Test, Deployers {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            address(this), flags, type(VolumeTrackerHook).creationCode, abi.encode(address(manager), "", 1, 0, address(token), dev, keeper)
        );
        hook = new VolumeTrackerHook{salt: salt}(IPoolManager(address(manager)), "", 1, 0, address(token), dev, keeper);
